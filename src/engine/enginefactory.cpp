...
bool EngineFactory::initialize(Backend backend) {
    ...
    if (!module.library) {
        ...
        if (!module.api || module.api->abiVersion != enginePluginAbiVersion || !module.api->initialize || !module.api->createView ||
            !module.api->createProfile || !module.api->shutdown) {
    ...
    module.initialized = module.api->initialize(static_cast<int>(state.argumentPointers.size()), state.argumentPointers.data());
