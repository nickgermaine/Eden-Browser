...
void CefUiBridge::assertOnUiThread(const QObject *stateOwner) {
    ...
    Q_ASSERT_X(QThread::currentThread() == application->thread(), "CefUiBridge::assertOnUiThread",
               "CEF callback accessed QObject state outside the Qt UI thread");
    Q_ASSERT_X(!stateOwner || stateOwner->thread() == QThread::currentThread(), "CefUiBridge::assertOnUiThread",
