...
bool CefRuntime::initialize(int argc, char *argv[], const std::filesystem::path &rootCachePath) {
    if (m_initialized) {
        return true;
    }
    m_exitCode = 0;
    ...
    const CefProcessSettingsResult processSettings =
        createCefProcessSettings(std::filesystem::absolute(argv[0]), std::span<const std::string_view>(arguments), m_rootCachePath);
