...
namespace {
    ...
    void
    appendListSwitch(CefRefPtr<CefCommandLine> commandLine, const std::string &switchName, const std::string &feature) {
...
namespace eden::engine::cef {
    ...
    CefApplication::OnBeforeCommandLineProcessing(const CefString &processType, CefRefPtr<CefCommandLine> commandLine) {
        ...
        commandLine->AppendSwitchWithValue("password-store", "gnome-libsecret");
        ...
        if (processType.empty() && !commandLine->HasSwitch("force-device-scale-factor") &&
            ...
            commandLine->AppendSwitchWithValue(
                ...
                QString::number(screen->devicePixelRatio()).toStdString()
            );
        }
    }
