...
class CefPopupNewViewRequest final : public EngineNewViewRequest {
  public:
    CefPopupNewViewRequest(const QUrl &url, Disposition disposition, bool userInitiated, std::shared_ptr<CefPopupTransfer> transfer)
...
class CefThumbnailObserver final : public CefDevToolsMessageObserver {
  public:
    ...
    void OnDevToolsMethodResult(CefRefPtr<CefBrowser>, int messageId, bool success, const void *result, size_t resultSize) override {
        ...
        if (success && result && resultSize > 0) {
            ...
            const QByteArray encoded = QJsonDocument::fromJson(payload).object().value("data").toString().toLatin1();
            image.loadFromData(QByteArray::fromBase64(encoded), "PNG");
            if (!image.isNull() && image.size() != m_size) {
                image = image.scaled(m_size, Qt::KeepAspectRatioByExpanding, Qt::SmoothTransformation);
                if (image.width() > m_size.width() || image.height() > m_size.height()) {
                    image = image.copy((image.width() - m_size.width()) / 2, (image.height() - m_size.height()) / 2, m_size.width(),
                                       m_size.height());
                }
            }
        }
  ...
  private:
    void start(CefRefPtr<CefBrowser> browser) {
        ...
        if (!m_registration || browser->GetHost()->ExecuteDevToolsMethod(m_messageId, "Page.captureScreenshot", parameters) == 0) {
            finish({});
        }
    }
...
class CefNavigationHistoryObserver final : public CefDevToolsMessageObserver {
  public:
    ...
    void OnDevToolsMethodResult(CefRefPtr<CefBrowser>, int messageId, bool success, const void *result, size_t resultSize) override {
        ...
        CefUiBridge::runOnUiThread([callback = std::move(callback), currentIndex, entries = std::move(entries)]() mutable {
            if (callback) {
  ...
  private:
    ...
    void start(CefRefPtr<CefBrowser> browser) {
        ...
        m_registration = browser->GetHost()->AddDevToolsMessageObserver(this);
        if (!m_registration ||
            browser->GetHost()->ExecuteDevToolsMethod(m_messageId, "Page.getNavigationHistory", CefDictionaryValue::Create()) == 0) {
            m_registration = nullptr;
        }
    }
...
class CefDevToolsClient final : public CefClient,
  ...
  public:
    explicit CefDevToolsClient(std::shared_ptr<CefViewLifetime> lifetime)
        : m_lifetime(std::move(lifetime)) {}
    ...
    bool OnPreKeyEvent(CefRefPtr<CefBrowser> browser, const CefKeyEvent &event, CefEventHandle osEvent, bool *isKeyboardShortcut) override;
    void OnLoadEnd(CefRefPtr<CefBrowser> browser, CefRefPtr<CefFrame> frame, int httpStatusCode) override;
    bool OnProcessMessageReceived(CefRefPtr<CefBrowser> browser, CefRefPtr<CefFrame> frame, CefProcessId sourceProcess,
    ...
    void OnPaint(CefRefPtr<CefBrowser> browser, PaintElementType type, const RectList &dirtyRects, const void *buffer, int width,
                 int height) override;
  ...
  private:
    template <typename Function> bool dispatch(Function function) {
        ...
        return CefUiBridge::runOnUiThread([lifetime, function = std::move(function)]() mutable {
            lifetime->runOrQueue(
                [function = std::move(function)](void *state) mutable { function(*static_cast<CefEngineView::Private *>(state)); });
        });
    }
...
class CefEngineClient final : public CefClient,
  ...
  public:
    ...
    bool OnBeforeDownload(CefRefPtr<CefBrowser> browser, CefRefPtr<CefDownloadItem> downloadItem, const CefString &suggestedName,
    ...
    void OnGotFocus(CefRefPtr<CefBrowser> browser) override;
    bool OnPreKeyEvent(CefRefPtr<CefBrowser> browser, const CefKeyEvent &event, CefEventHandle osEvent, bool *isKeyboardShortcut) override;
    void OnLoadingStateChange(CefRefPtr<CefBrowser> browser, bool loading, bool canGoBack, bool canGoForward) override;
    void OnLoadEnd(CefRefPtr<CefBrowser> browser, CefRefPtr<CefFrame> frame, int httpStatusCode) override;
    ...
    bool OnProcessMessageReceived(CefRefPtr<CefBrowser> browser, CefRefPtr<CefFrame> frame, CefProcessId sourceProcess,
                                  CefRefPtr<CefProcessMessage> message) override;
    void OnMainFrameChanged(CefRefPtr<CefBrowser> browser, CefRefPtr<CefFrame> oldFrame, CefRefPtr<CefFrame> newFrame) override;
    void OnFrameDestroyed(CefRefPtr<CefBrowser> browser, CefRefPtr<CefFrame> frame) override;
    bool OnBeforePopup(CefRefPtr<CefBrowser> browser, CefRefPtr<CefFrame> frame, int popupId, const CefString &targetUrl,
                       const CefString &targetFrameName, CefLifeSpanHandler::WindowOpenDisposition targetDisposition, bool userGesture,
                       const CefPopupFeatures &popupFeatures, CefWindowInfo &windowInfo, CefRefPtr<CefClient> &client,
                       CefBrowserSettings &settings, CefRefPtr<CefDictionaryValue> &extraInfo, bool *noJavascriptAccess) override;
    void OnBeforePopupAborted(CefRefPtr<CefBrowser> browser, int popupId) override;
    bool OnJSDialog(CefRefPtr<CefBrowser> browser, const CefString &originUrl, JSDialogType dialogType, const CefString &messageText,
                    const CefString &defaultPromptText, CefRefPtr<CefJSDialogCallback> callback, bool &suppressMessage) override;
    bool OnBeforeUnloadDialog(CefRefPtr<CefBrowser> browser, const CefString &messageText, bool isReload,
                              CefRefPtr<CefJSDialogCallback> callback) override;
    void OnResetDialogState(CefRefPtr<CefBrowser> browser) override;
    bool OnRequestMediaAccessPermission(CefRefPtr<CefBrowser> browser, CefRefPtr<CefFrame> frame, const CefString &requestingOrigin,
                                        uint32_t requestedPermissions, CefRefPtr<CefMediaAccessCallback> callback) override;
    bool OnShowPermissionPrompt(CefRefPtr<CefBrowser> browser, uint64_t promptId, const CefString &requestingOrigin,
                                uint32_t requestedPermissions, CefRefPtr<CefPermissionPromptCallback> callback) override;
    void OnDismissPermissionPrompt(CefRefPtr<CefBrowser> browser, uint64_t promptId, cef_permission_request_result_t result) override;
    bool OnFileDialog(CefRefPtr<CefBrowser> browser, FileDialogMode mode, const CefString &title, const CefString &defaultFilePath,
                      const std::vector<CefString> &acceptFilters, const std::vector<CefString> &acceptExtensions,
                      const std::vector<CefString> &acceptDescriptions, CefRefPtr<CefFileDialogCallback> callback) override;
    void GetViewRect(CefRefPtr<CefBrowser> browser, CefRect &rect) override;
    bool GetScreenInfo(CefRefPtr<CefBrowser> browser, CefScreenInfo &screenInfo) override;
    void OnPaint(CefRefPtr<CefBrowser> browser, PaintElementType type, const RectList &dirtyRects, const void *buffer, int width,
                 int height) override;
...
class CefEngineView::Private {
  public:
    ...
    void attachDevTools(QQuickItem *item) {
        ...
        devToolsViewportConnections.append(QObject::connect(devToolsViewport, &QQuickItem::windowChanged, q,
                                                            [this](QQuickWindow *) { attachDevTools(devToolsViewport); }));
        ...
        for (QQuickItem *ancestor = devToolsViewport; ancestor; ancestor = ancestor->parentItem()) {
            const auto connectGeometry = [this, ancestor](auto signal) {
                devToolsViewportConnections.append(QObject::connect(ancestor, signal, q, [this] { updateDevToolsGeometry(); }));
            };
            ...
            devToolsViewportConnections.append(
                QObject::connect(ancestor, &QQuickItem::parentChanged, q, [this](QQuickItem *) { attachDevTools(devToolsViewport); }));
        }
        devToolsViewportConnections.append(
        ...
        devToolsViewportConnections.append(
            QObject::connect(shellWindow, &QQuickWindow::widthChanged, q, [this] { updateDevToolsGeometry(); }));
        devToolsViewportConnections.append(
            QObject::connect(shellWindow, &QQuickWindow::heightChanged, q, [this] { updateDevToolsGeometry(); }));
        devToolsViewportConnections.append(
            QObject::connect(shellWindow, &QQuickWindow::visibleChanged, q, [this](bool) { syncDevToolsVisibility(); }));
        devToolsViewportConnections.append(QObject::connect(shellWindow, &QQuickWindow::activeChanged, q, [this] { syncDevToolsFocus(); }));
        devToolsViewportConnections.append(
            QObject::connect(shellWindow, &QQuickWindow::devicePixelRatioChanged, q, [this] { updateDevToolsGeometry(); }));
    ...
    void attachViewport() {
        ...
        viewportConnections.append(QObject::connect(viewport, &QQuickItem::windowChanged, q, [this](QQuickWindow *) { attachViewport(); }));
        if (!viewport->window()) {
            return;
        }
        ...
        for (QQuickItem *item = viewport; item; item = item->parentItem()) {
            ...
            connectGeometry(&QQuickItem::widthChanged);
            connectGeometry(&QQuickItem::heightChanged);
            viewportConnections.append(QObject::connect(item, &QQuickItem::parentChanged, q, [this](QQuickItem *) { attachViewport(); }));
        }
        viewportConnections.append(QObject::connect(viewport, &QQuickItem::visibleChanged, q, [this] { syncVisibility(); }));
        viewportConnections.append(QObject::connect(viewport, &QQuickItem::activeFocusChanged, q, [this] { syncFocus(); }));
        viewportConnections.append(QObject::connect(shellWindow, &QQuickWindow::widthChanged, q, [this] { updateGeometry(); }));
        viewportConnections.append(QObject::connect(shellWindow, &QQuickWindow::heightChanged, q, [this] { updateGeometry(); }));
        viewportConnections.append(QObject::connect(shellWindow, &QQuickWindow::visibleChanged, q, [this](bool) { syncVisibility(); }));
        viewportConnections.append(QObject::connect(shellWindow, &QQuickWindow::activeChanged, q, [this] { syncFocus(); }));
        viewportConnections.append(QObject::connect(shellWindow, &QQuickWindow::devicePixelRatioChanged, q, [this] { updateGeometry(); }));
        updateGeometry();
        syncVisibility();
        createBrowser();
    }
    ...
    void createBrowser() {
        ...
        if (osr) {
            ...
            hostWindow->create();
            windowInfo.SetAsChild(static_cast<CefWindowHandle>(hostWindow->winId()),
        ...
        if (!postToCefUi([windowInfo, currentClient, requestContext, initialUrl]() mutable {
                ...
                if (!CefBrowserHost::CreateBrowser(windowInfo, currentClient, initialUrl, settings, nullptr, requestContext)) {
    ...
    void createDevToolsBrowser() {
        if (!q->devToolsOpen() || devToolsBrowser || devToolsBrowserCreationStarted || !browser || !profile || !devToolsViewport ||
            !devToolsOsrItem) {
            return;
        }
        ...
        devToolsClient = new CefDevToolsClient(devToolsLifetime);
    ...
    void createDevToolsFrontendBrowser() {
        CefUiBridge::assertOnUiThread(q);
        if (!q->devToolsOpen() || devToolsBrowser || !devToolsBrowserCreationStarted || !devToolsClient || !profile ||
            !devToolsSocketSession.isValid()) {
            return;
        }
        updateDevToolsGeometry();
        CefWindowInfo windowInfo;
        ...
        const std::string frontendUrl =
            QString("devtools://devtools/bundled/inspector.html?ws=%1&panel=elements").arg(devToolsSocketSession.endpoint).toStdString();
        if (!currentClient->beginBrowserCreation()) {
            destroyDevTools();
            return;
        }
        if (!postToCefUi([windowInfo, currentClient, requestContext, frontendUrl]() mutable {
                ...
                settings.background_color = CefColorSetARGB(255, 255, 255, 255);
                settings.windowless_frame_rate = 60;
                if (!CefBrowserHost::CreateBrowser(windowInfo, currentClient, frontendUrl, settings, nullptr, requestContext)) {
                    currentClient->browserCreationFailed();
                }
            })) {
            currentClient->browserCreationFailed();
    ...
    void updateLoading(bool nextLoading, bool nextCanGoBack, bool nextCanGoForward) {
        ...
        const int nextProgress = loading ? 0 : 100;
        if (progress != nextProgress) {
            progress = nextProgress;
            emit q->loadProgressChanged();
        }
    ...
    void scheduleRendererGarbageCollection() {
        CefUiBridge::assertOnUiThread(q);
        if (rendererGcPending) {
            return;
        }
        rendererGcPending = true;
        QPointer<CefEngineView> guard(q);
        QTimer::singleShot(1500, q, [guard] {
            if (!guard) {
                return;
            }
            guard->d->rendererGcPending = false;
            const CefRefPtr<CefBrowser> currentBrowser = guard->d->browser;
            if (!currentBrowser || guard->d->loading) {
                return;
            }
            postToCefUi([currentBrowser] {
                currentBrowser->GetHost()->ExecuteDevToolsMethod(nextDevToolsMessageId(), "HeapProfiler.collectGarbage", nullptr);
            });
        });
    }
    ...
    void requestNewView(const QUrl &requestedUrl, EngineView::Disposition disposition, bool userInitiated,
    ...
    void updateDownload(uint32_t id, const QString &fileName, const QUrl &sourceUrl, const QString &targetPath, qint64 receivedBytes,
                        qint64 totalBytes, const QString &state, bool firstUpdate) {
        CefUiBridge::assertOnUiThread(q);
        if (!profile) {
            return;
        }
    ...
    void installFileChooserInterception() {
        ...
        postToCefUi([currentBrowser, observer, guard] {
            CefRefPtr<CefRegistration> registration = currentBrowser->GetHost()->AddDevToolsMessageObserver(observer);
            currentBrowser->GetHost()->ExecuteDevToolsMethod(nextDevToolsMessageId(), "Page.enable", nullptr);
            CefRefPtr<CefDictionaryValue> parameters = CefDictionaryValue::Create();
            parameters->SetBool("enabled", true);
            currentBrowser->GetHost()->ExecuteDevToolsMethod(nextDevToolsMessageId(), "Page.setInterceptFileChooserDialog", parameters);
    ...
    bool resolveFileChooser(quint64 id, bool accepted, const QList<QUrl> &files) {
        ...
        postToCefUi([currentBrowser, backendNodeId, frameId, paths] {
            ...
            currentBrowser->GetHost()->ExecuteDevToolsMethod(nextDevToolsMessageId(), "DOM.setFileInputFiles", parameters);
        });
        return true;
    }
    ...
    void pasteFromShellClipboard() {
        ...
        postToCefUi([currentBrowser, encoded] {
            ...
            currentBrowser->GetHost()->ExecuteDevToolsMethod(nextDevToolsMessageId(), "Input.insertText", parameters);
        });
    }
    ...
    void devToolsBrowserFocused() {
        CefUiBridge::assertOnUiThread(q);
        if (!devToolsBrowser || !devToolsViewport || !devToolsOsrItem || !devToolsViewport->isVisible() || !devToolsOsrItem->isVisible()) {
            return;
        }
        osrPageFocused = false;
    ...
    void updateGeometry() {
        ...
        client->setRootWindowScreenRect(CefRect(screenGeometry.x(), screenGeometry.y(), screenGeometry.width(), screenGeometry.height()));
        client->setOsrGeometry(CefRect(0, 0, browserLogicalSize().width(), browserLogicalSize().height()),
    ...
    void updateDevToolsGeometry() {
        ...
        if (devToolsClient) {
            ...
            devToolsClient->setGeometry(CefRect(screenGeometry.x(), screenGeometry.y(), screenGeometry.width(), screenGeometry.height()),
    ...
    QSize browserPixelSize() const {
        ...
        return QSize(std::max(1, qRound(viewport->width() * scale)), std::max(1, qRound(viewport->height() * scale)));
    }
    ...
    bool automationWheel(int delta) {
        ...
        const QByteArray message =
            ...
            ",\"method\":\"Input.dispatchMouseEvent\",\"params\":{\"type\":\"mouseWheel\",\"x\":" + QByteArray::number(size.width() / 2) +
            ",\"y\":" + QByteArray::number(size.height() / 2) + ",\"deltaX\":0,\"deltaY\":" + QByteArray::number(-delta) +
            ",\"pointerType\":\"mouse\"}}";
        const CefRefPtr<CefBrowser> target = browser;
        return postToCefUi(
            [target, message] { target->GetHost()->SendDevToolsMessage(message.constData(), static_cast<size_t>(message.size())); });
    }
#endif
    ...
    QSize devToolsLogicalSize() const {
        ...
        return QSize(std::max(1, qRound(devToolsViewport->width())), std::max(1, qRound(devToolsViewport->height())));
    }
    ...
    void resizeBrowser() {
        ...
        postToCefUi([currentBrowser, pixelSize] {
            ...
            if (display && browserWindow && !pixelSize.isEmpty()) {
                XMoveResizeWindow(display, browserWindow, 0, 0, static_cast<unsigned int>(pixelSize.width()),
    ...
    void syncVisibility() {
        ...
        const bool visible = viewport && viewport->window() && viewport->window()->isVisible() && viewport->isVisible() &&
    ...
    void syncDevToolsVisibility() {
        ...
        const bool visible = q->devToolsOpen() && devToolsViewport && devToolsViewport->window() &&
                             devToolsViewport->window()->isVisible() && devToolsViewport->isVisible() && devToolsViewport->width() > 0 &&
    ...
    bool forwardDevToolsOsrEvent(QEvent *event) {
        return forwardOsrEventToBrowser(event, devToolsBrowser, devToolsOsrItem, devToolsPageFocused, osrPageFocused);
    }

    bool forwardOsrEventToBrowser(QEvent *event, CefRefPtr<CefBrowser> targetBrowser, CefOsrItem *targetItem, bool &focused,
                                  bool &otherFocused) {
        ...
        if (event->type() == QEvent::MouseMove) {
            ...
            postToCefUi([currentBrowser, cefEvent] { currentBrowser->GetHost()->SendMouseMoveEvent(cefEvent, false); });
            return true;
        }
        if (event->type() == QEvent::HoverMove) {
            ...
            cefEvent.modifiers = cefMouseModifiers(QGuiApplication::keyboardModifiers(), QGuiApplication::mouseButtons());
            postToCefUi([currentBrowser, cefEvent] { currentBrowser->GetHost()->SendMouseMoveEvent(cefEvent, false); });
            return true;
        }
        if (event->type() == QEvent::Leave || event->type() == QEvent::HoverLeave) {
            ...
            CefMouseEvent cefEvent;
            postToCefUi([currentBrowser, cefEvent] { currentBrowser->GetHost()->SendMouseMoveEvent(cefEvent, true); });
            return true;
        }
        ...
        if (event->type() == QEvent::Wheel) {
            ...
            postToCefUi(
                [currentBrowser, cefEvent, delta] { currentBrowser->GetHost()->SendMouseWheelEvent(cefEvent, delta.x(), delta.y()); });
            return true;
        }
        if (event->type() == QEvent::KeyPress || event->type() == QEvent::KeyRelease) {
            ...
            postToCefUi([currentBrowser, cefEvent, text] {
                ...
                if (cefEvent.type == KEYEVENT_RAWKEYDOWN && !text.isEmpty() && !(cefEvent.modifiers & EVENTFLAG_CONTROL_DOWN) &&
    ...
    bool acceptsOsrKeyboardInput() const {
        return osrPageFocused && osrEventWindow && osrEventWindow->isActive() && viewport && viewport->isVisible() && osrItem &&
               osrItem->isVisible();
    }
...
bool CefFileChooserObserver::OnDevToolsMessage(CefRefPtr<CefBrowser>, const void *message, size_t messageSize) {
    if (!message || messageSize == 0) {
        return false;
    }
    const QByteArray payload(static_cast<const char *>(message), static_cast<qsizetype>(messageSize));
    const QJsonObject event = QJsonDocument::fromJson(payload).object();
    if (event.value("method").toString() != "Page.fileChooserOpened") {
        return false;
    }
    const QJsonObject parameters = event.value("params").toObject();
...
bool CefDevToolsMessageForwarder::OnDevToolsMessage(CefRefPtr<CefBrowser>, const void *message, size_t messageSize) {
    if (!message || messageSize == 0) {
        return false;
    }
    if (const std::shared_ptr<CefDevToolsProtocolSession> session = m_session.lock()) {
        session->deliverToFrontend(QByteArray(static_cast<const char *>(message), static_cast<qsizetype>(messageSize)));
        return true;
    }
    return false;
}
...
void CefDevToolsProtocolSession::sendToInspected(QByteArray message) {
    ...
    postToCefUi([self, message = std::move(message)] {
        if (!self->m_closed.load(std::memory_order_acquire) && self->m_browser) {
            self->m_browser->GetHost()->SendDevToolsMessage(message.constData(), static_cast<size_t>(message.size()));
        }
    });
}
...
void CefDevToolsProtocolSession::deliverToFrontend(QByteArray message) {
    ...
    CefUiBridge::runOnUiThread(
        [token, message = std::move(message)]() mutable { sharedDevToolsSocketServer()->sendMessage(token, message); });
}
...
bool CefDevToolsClient::OnCursorChange(CefRefPtr<CefBrowser>, CefCursorHandle, cef_cursor_type_t type, const CefCursorInfo &) {
    if (type == CT_CUSTOM) {
        return false;
    }
...
bool CefDevToolsClient::OnPreKeyEvent(CefRefPtr<CefBrowser>, const CefKeyEvent &event, CefEventHandle, bool *) {
...
bool CefDevToolsClient::OnProcessMessageReceived(CefRefPtr<CefBrowser>, CefRefPtr<CefFrame>, CefProcessId,
...
void CefDevToolsClient::OnLoadEnd(CefRefPtr<CefBrowser>, CefRefPtr<CefFrame> frame, int) {
    ...
    frame->ExecuteJavaScript("(function(){try{"
...
void CefDevToolsClient::OnPaint(CefRefPtr<CefBrowser>, PaintElementType type, const RectList &, const void *buffer, int width, int height) {
    if (type != PET_VIEW || !buffer || width <= 0 || height <= 0) {
        return;
    }
    QImage frame(static_cast<const uchar *>(buffer), width, height, width * 4, QImage::Format_ARGB32);
    enqueueFrame(frame.copy());
}
...
void CefDevToolsClient::closeAndWait() {
    ...
    m_browserCondition.wait_for(lock, std::chrono::seconds(5), [this] { return m_closed && !m_creationPending && !m_browser; });
}
...
void CefDevToolsClient::setGeometry(const CefRect &rootWindowScreenRect, const CefRect &viewRect, float deviceScaleFactor) {
    const std::lock_guard lock(m_screenRectMutex);
...
void CefEngineClient::OnBeforeClose(CefRefPtr<CefBrowser> browser) {
    ...
    if (qEnvironmentVariableIsSet("EDEN_IDLE_DIAGNOSTICS")) {
        qInfo("EDEN_PERF diagnostic.osr_paints=%llu", static_cast<unsigned long long>(m_idleDiagnosticPaints.load()));
    }
#endif
    cancelInteractions();
    {
...
void CefEngineClient::OnAddressChange(CefRefPtr<CefBrowser>, CefRefPtr<CefFrame> frame, const CefString &url) {
    ...
    dispatch([value](CefEngineView::Private &state) { state.updateUrl(value); });
}
...
void CefEngineClient::OnTitleChange(CefRefPtr<CefBrowser>, const CefString &title) {
    ...
    if (value.startsWith("bench:wheel:") || value.startsWith("bench:scroll:")) {
        const std::optional<double> pageEvent = eden::core::PerformanceMetrics::complete("scroll.input_to_page_event_ms");
...
void CefEngineClient::OnFaviconURLChange(CefRefPtr<CefBrowser> browser, const std::vector<CefString> &iconUrls) {
    ...
    for (const CefString &iconUrl : iconUrls) {
        m_faviconCandidates.push_back(iconUrl.ToString());
    }
...
void CefEngineClient::requestFaviconCandidate(CefRefPtr<CefBrowser> browser, quint64 serial) {
    ...
    browser->GetHost()->DownloadImage(m_faviconCandidates.at(m_faviconCandidateIndex), true, 128, false,
...
bool CefEngineClient::OnCursorChange(CefRefPtr<CefBrowser>, CefCursorHandle, cef_cursor_type_t type, const CefCursorInfo &) {
    if (type == CT_CUSTOM) {
        return false;
    }
...
bool CefEngineClient::RunContextMenu(CefRefPtr<CefBrowser>, CefRefPtr<CefFrame>, CefRefPtr<CefContextMenuParams> params,
    ...
    m_contextMenuCommands = {
        {"back", MENU_ID_BACK}, {"forward", MENU_ID_FORWARD}, {"reload", MENU_ID_RELOAD},         {"cut", MENU_ID_CUT},
        {"copy", MENU_ID_COPY}, {"paste", MENU_ID_PASTE},     {"select_all", MENU_ID_SELECT_ALL}, {"view_source", MENU_ID_VIEW_SOURCE}};
    if (model) {
...
bool CefEngineClient::OnBeforeDownload(CefRefPtr<CefBrowser>, CefRefPtr<CefDownloadItem> downloadItem, const CefString &suggestedName,
...
void CefEngineClient::OnDownloadUpdated(CefRefPtr<CefBrowser>, CefRefPtr<CefDownloadItem> downloadItem,
...
bool CefEngineClient::OnPreKeyEvent(CefRefPtr<CefBrowser> browser, const CefKeyEvent &event, CefEventHandle, bool *) {
    const CefWindowHandle browserWindow = browser->GetHost()->GetWindowHandle();
    if (browserWindow) {
        Display *display = cef_get_xdisplay();
        Window focusedWindow = 0;
...
void CefEngineClient::OnLoadingStateChange(CefRefPtr<CefBrowser>, bool loading, bool canGoBack, bool canGoForward) {
    dispatch([loading, canGoBack, canGoForward](CefEngineView::Private &state) { state.updateLoading(loading, canGoBack, canGoForward); });
}

void CefEngineClient::OnLoadEnd(CefRefPtr<CefBrowser>, CefRefPtr<CefFrame> frame, int) {
    if (frame && frame->IsMain()) {
#if EDEN_ENABLE_AUTOMATION
...
#endif
        dispatch([](CefEngineView::Private &state) { state.loadFinished(); });
    }
}

bool CefEngineClient::OnProcessMessageReceived(CefRefPtr<CefBrowser>, CefRefPtr<CefFrame> frame, CefProcessId,
                                               CefRefPtr<CefProcessMessage> message) {
    if (!message) {
        return false;
    }
...
void CefEngineClient::OnFrameDestroyed(CefRefPtr<CefBrowser>, CefRefPtr<CefFrame> frame) {
    ...
    const std::lock_guard lock(m_framePidMutex);
    m_framePids.erase(frame->GetIdentifier().ToString());
}

void CefEngineClient::OnLoadError(CefRefPtr<CefBrowser>, CefRefPtr<CefFrame> frame, ErrorCode errorCode, const CefString &errorText,
    ...
    const QString html =
        QString("<!doctype html><html><head><meta charset=utf-8><meta name=viewport content=\"width=device-width,initial-scale=1\">"
                ...
                "font-family:system-ui,sans-serif;background:#f6f8f4;color:#1a1c19}.card{box-sizing:border-box;width:min(560px,calc(100% "
                "- 48px));"
                ...
                "color:#fff;text-decoration:none;font-weight:650}@media(prefers-color-scheme:dark){body{background:#10140f;color:#e1e4dc}."
                "card{background:#1b211a}"
                ".detail{color:#bbc6b7}a{background:#a5d6a7;color:#173a1c}}</style></head><body><main class=card><h1>This page could not "
                "be reached</h1>"
    ...
    const QString dataUrl = QString("data:text/html;charset=utf-8,%1").arg(QString::fromLatin1(QUrl::toPercentEncoding(html)));
    m_errorPageUrl = dataUrl;
    m_failedUrl = failed;
    frame->LoadURL(dataUrl.toStdString());
}

bool CefEngineClient::OnBeforePopup(CefRefPtr<CefBrowser>, CefRefPtr<CefFrame>, int popupId, const CefString &targetUrl, const CefString &,
    ...
    } else if (targetDisposition == CEF_WOD_NEW_WINDOW || targetDisposition == CEF_WOD_NEW_POPUP || popupFeatures.isPopup) {
...
bool CefEngineClient::OnJSDialog(CefRefPtr<CefBrowser>, const CefString &originUrl, JSDialogType dialogType, const CefString &messageText,
                                 const CefString &defaultPromptText, CefRefPtr<CefJSDialogCallback> callback, bool &suppressMessage) {
    ...
    info.kind = dialogType == JSDIALOGTYPE_PROMPT ? "prompt" : dialogType == JSDIALOGTYPE_CONFIRM ? "confirm" : "alert";
...
bool CefEngineClient::OnBeforeUnloadDialog(CefRefPtr<CefBrowser>, const CefString &messageText, bool,
...
bool CefEngineClient::OnRequestMediaAccessPermission(CefRefPtr<CefBrowser>, CefRefPtr<CefFrame>, const CefString &requestingOrigin,
...
bool CefEngineClient::OnShowPermissionPrompt(CefRefPtr<CefBrowser>, uint64_t promptId, const CefString &requestingOrigin,
...
bool CefEngineClient::OnFileDialog(CefRefPtr<CefBrowser>, FileDialogMode mode, const CefString &title, const CefString &defaultFilePath,
                                   const std::vector<CefString> &acceptFilters, const std::vector<CefString> &acceptExtensions,
                                   const std::vector<CefString> &acceptDescriptions, CefRefPtr<CefFileDialogCallback> callback) {
    if (!callback) {
        return true;
    }
    const quint64 id = m_nextInteractionId++;
    ...
    for (std::size_t index = 0; index < acceptFilters.size(); ++index) {
        ...
        const QString extensions =
            index < acceptExtensions.size() ? QString::fromStdString(acceptExtensions.at(index).ToString()) : QString();
        const QString description =
            index < acceptDescriptions.size() ? QString::fromStdString(acceptDescriptions.at(index).ToString()).trimmed() : QString();
...
void CefEngineClient::OnDismissPermissionPrompt(CefRefPtr<CefBrowser>, uint64_t promptId, cef_permission_request_result_t) {
...
void CefEngineClient::OnPaint(CefRefPtr<CefBrowser>, PaintElementType type, const RectList &dirtyRects, const void *buffer, int width,
                              int height) {
    if (type != PET_VIEW || !buffer || width <= 0 || height <= 0) {
        return;
    }
    ...
    QImage stagedFrame;
    {
        ...
        for (const QRect &copyRect : copyRects) {
            ...
            for (int row = copyRect.top(); row <= copyRect.bottom(); ++row) {
                std::memcpy(target.scanLine(row) + columnOffset, source + static_cast<qsizetype>(row) * sourceStride + columnOffset,
...
void CefEngineClient::OnAcceleratedPaint(CefRefPtr<CefBrowser>, PaintElementType type, const RectList &,
                                         const CefAcceleratedPaintInfo &info) {
    if (type != PET_VIEW || !qEnvironmentVariableIsSet("EDEN_PERF")) {
        return;
    }
    qInfo("EDEN_PERF osr.accelerated_paint planes=%d modifier=%llu format=%d width=%d height=%d", info.plane_count,
          static_cast<unsigned long long>(info.modifier), static_cast<int>(info.format), info.extra.coded_size.width,
          info.extra.coded_size.height);
}

void CefEngineClient::OnRenderProcessTerminated(CefRefPtr<CefBrowser> browser, TerminationStatus status, int errorCode,
    ...
    const QString html =
        QString(
            ...
            "<title>Tab crashed</title><style>html{color-scheme:light dark}body{margin:0;min-height:100vh;display:grid;place-items:center;"
            "font-family:system-ui,sans-serif;background:#f6f8f4;color:#1a1c19}.card{box-sizing:border-box;width:min(560px,calc(100% "
            "- 48px));padding:40px;border-radius:24px;background:#fff;box-shadow:0 12px 36px #0002}h1{margin:0 0 12px;font-size:30px}"
            "p{line-height:1.55}.detail{color:#586056;overflow-wrap:anywhere}a{display:inline-block;margin-top:16px;padding:12px "
            "20px;border-radius:24px;background:#315d32;color:#fff;text-decoration:none;font-weight:650}"
            "@media(prefers-color-scheme:dark){body{background:#10140f;color:#e1e4dc}.card{background:#1b211a}.detail{color:#bbc6b7}"
            "a{background:#a5d6a7;color:#173a1c}}</style></head><body><main class=card><h1>This tab stopped working</h1>"
            "<p>The page renderer ended unexpectedly. Eden and your other tabs are still running.</p><p class=detail>%1</p>"
            "<a href=\"%2\">Reload page</a></main></body></html>")
            .arg(details.toHtmlEscaped(), failedUrl.toString().toHtmlEscaped());
    const QString dataUrl = QString("data:text/html;charset=utf-8,%1").arg(QString::fromLatin1(QUrl::toPercentEncoding(html)));
    dispatch([failedUrl, details, dataUrl](CefEngineView::Private &state) { state.renderProcessTerminated(failedUrl, details, dataUrl); });
}
...
CefEngineClient::DownloadRecord &CefEngineClient::downloadRecord(CefRefPtr<CefDownloadItem> downloadItem, const QString &suggestedName) {
    const uint32_t id = downloadItem->GetId();
    DownloadRecord &download = m_downloads[id];
    if (download.fileName.isEmpty()) {
        const QString candidate =
            suggestedName.isEmpty() ? QString::fromStdString(downloadItem->GetSuggestedFileName().ToString()) : suggestedName;
        ...
        const QString baseName = nameInfo.completeBaseName().isEmpty() ? QString("download") : nameInfo.completeBaseName();
...
void CefEngineClient::publishDownload(uint32_t id, DownloadRecord &download) {
    ...
    dispatch([id, snapshot, firstUpdate](CefEngineView::Private &view) {
        view.updateDownload(id, snapshot.fileName, snapshot.sourceUrl, snapshot.targetPath, snapshot.receivedBytes, snapshot.totalBytes,
                            snapshot.state, firstUpdate);
    });
}
...
static void collectDescendantProcesses(qint64 processId, QList<qint64> &result) {
    const QStringList tasks = QDir(QString("/proc/%1/task").arg(processId)).entryList(QDir::Dirs | QDir::NoDotAndDotDot);
...
QVariantList CefEngineView::navigationHistory(int direction, int maximumItems) const {
    ...
    for (int distance = 1; distance <= boundedMaximum; ++distance) {
        ...
        const QString url = aliasEngineInternalUrl(QUrl(item.value("url").toString())).toDisplayString(QUrl::RemovePassword);
...
void CefEngineView::goToHistoryOffset(int offset) {
    ...
    if (d->historyCurrentIndex >= 0 && index >= 0 && index < d->historyEntries.size()) {
        ...
        postToCefUi([currentBrowser, entryId] {
            ...
            currentBrowser->GetHost()->ExecuteDevToolsMethod(nextDevToolsMessageId(), "Page.navigateToHistoryEntry", parameters);
        });
        return;
    }
    d->browser->GetMainFrame()->ExecuteJavaScript(QString("history.go(%1)").arg(offset).toStdString(), d->url.toString().toStdString(), 0);
}
