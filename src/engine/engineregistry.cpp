...
namespace eden::engine {
    ...
    EngineRegistry::EngineRegistry(QList<EngineDescriptor> descriptors, QObject *parent)
        ...
        for (EngineDescriptor &descriptor : descriptors) {
            ...
            if (descriptor.id.isEmpty() || descriptor.displayName.isEmpty() || contains(descriptor.backend) ||
    ...
    bool EngineRegistry::contains(Backend backend) const {
        return std::any_of(m_descriptors.cbegin(), m_descriptors.cend(), [backend](const EngineDescriptor &descriptor) {
            return descriptor.backend == backend;
        });
    }
    ...
    QString EngineRegistry::idForBackend(Backend backend) const {
        const auto found =
            std::find_if(m_descriptors.cbegin(), m_descriptors.cend(), [backend](const EngineDescriptor &descriptor) {
                return descriptor.backend == backend;
            });
    ...
    QVariantList EngineRegistry::selectionActions(const QString &selectedId) const {
        ...
        for (const EngineDescriptor &descriptor : m_descriptors) {
            ...
            action.insert("icon", descriptor.id == selectedId ? QString("check") : QString("programming"));
