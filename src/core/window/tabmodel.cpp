...
namespace eden::core {
    ...
    QString TabModel::internalPageForUrl(const QUrl &url) {
        ...
        if (host == "settings" || host == "theme-editor" || host == "newtab") {
            return host;
        }
        return {};
    }
    ...
    TabModel::TabModel(
        LegacyViewFactory factory,
        bool privateMode,
        QObject *parent,
        std::shared_ptr<engine::EngineProfile> profileLease
    )
        : TabModel(
              [factory = std::move(factory)](engine::Backend, engine::EngineProfile *) {
                  return factory ? factory() : nullptr;
              },
              [profileLease = std::move(profileLease)](engine::Backend) { return profileLease; },
              engine::EngineRegistry::instance(),
              engine::Backend::QtWebEngine,
              privateMode,
              {},
              parent
          ) {}
    ...
    TabModel::TabModel(
        ...
        const engine::EngineRegistry *registry,
    ...
    void TabModel::setConversionGate(ConversionGate gate) {
        m_conversionGate = std::move(gate);
    }

    const QString &TabModel::profileToken() const {
    ...
    int TabModel::addTab(const QUrl &url, bool background, const QString &backendId) {
        ...
        tab->url = url.isEmpty() ? QUrl("eden://newtab") : url;
    ...
    bool TabModel::undoClose() {
        ...
        const int row = restoredPinned ? std::clamp(m_closedTab->index, 0, pinnedCount())
                                       : std::clamp(m_closedTab->index, pinnedCount(), rowCount());
        beginInsertRows({}, row, row);
    ...
    bool TabModel::convertTab(int row, const QString &backendId, bool rehydrate) {
        ...
        const std::shared_ptr<engine::EngineProfile> sourceLease = tab.profileLease;
        discardTab(row);
        tab.backend = *backend;
        bool created = true;
        if (rehydrate && tab.internalPage.isEmpty()) {
            if (m_conversionGate && !m_privateMode && sourceLease) {
                rehydrateThroughGate(sourceLease, *backend, tab.id);
            } else {
    ...
    void TabModel::rehydrateThroughGate(
        std::shared_ptr<engine::EngineProfile> sourceLease,
        engine::Backend destination,
        quint64 tabId
    ) {
        QPointer<TabModel> guard(this);
        m_conversionGate(std::move(sourceLease), destination, [guard, tabId, destination] {
            if (!guard) {
                return;
            }
            const int currentRow = guard->indexForTabId(tabId);
            if (currentRow < 0) {
                return;
            }
            const Tab &tab = *guard->m_tabs.at(currentRow);
            if (tab.backend != destination || tab.view) {
                return;
            }
            guard->createView(currentRow);
        });
    }

    int TabModel::openWithTab(const QUrl &url, const QString &backendId, int sourceIndex) {
        const std::optional<engine::Backend> backend = m_registry->backendForId(backendId);
        if (!backend) {
            return -1;
        }
        std::shared_ptr<engine::EngineProfile> sourceLease;
        engine::Backend sourceBackend = m_defaultBackend;
        if (sourceIndex >= 0 && sourceIndex < rowCount()) {
            sourceLease = m_tabs.at(sourceIndex)->profileLease;
            sourceBackend = m_tabs.at(sourceIndex)->backend;
        }
        const bool gated = m_conversionGate && !m_privateMode && sourceLease && sourceBackend != *backend;
        if (!gated) {
            return addTab(url, false, backendId);
        }
        std::unique_ptr<Tab> tab = std::make_unique<Tab>();
        tab->id = nextTabId();
        tab->privateMode = m_privateMode;
        ...
        emit countChanged();
        emit operationOccurred();
        emit tabAdded(row, false);
        if (m_tabs.back()->internalPage.isEmpty()) {
            rehydrateThroughGate(std::move(sourceLease), *backend, tabId);
        }
        return row;
    }
    ...
    bool TabModel::transferTabTo(int row, TabModel *destination, int destinationRow) {
        if (!destination || row < 0 || row >= rowCount() || m_privateMode != destination->m_privateMode ||
            m_profileToken != destination->m_profileToken ||
        ...
        if (destination == this) {
            ...
            target = transferredPinned ? std::clamp(target, 0, std::max(0, pinnedCount() - 1))
                                       : std::clamp(target, pinnedCount(), rowCount() - 1);
            return target == row || moveTab(row, target);
        }
        destinationRow = transferredPinned
                             ? std::clamp(destinationRow, 0, destination->pinnedCount())
    ...
    void TabModel::restoreTabs(const QJsonArray &tabs) {
        for (const QJsonValue &value : tabs) {
            ...
            restoreTab(
                urlWithoutCredentials(QUrl(tab.value("url").toString())),
                tab.value("title").toString(),
                tab.value("pinned").toBool(),
    ...
    int TabModel::pinnedCount() const {
        return static_cast<int>(std::count_if(m_tabs.cbegin(), m_tabs.cend(), [](const std::unique_ptr<Tab> &tab) {
            return tab->pinned;
        }));
    }
    ...
    int TabModel::rendererProcessUseCount(qint64 processId) const {
        ...
        return static_cast<int>(
            std::count_if(m_tabs.cbegin(), m_tabs.cend(), [processId](const std::unique_ptr<Tab> &tab) {
    ...
    void TabModel::connectTab(Tab &tab) {
        ...
        connect(view, &engine::EngineView::urlChanged, this, [this, view] {
            notifyViewChanged(view, {UrlRole, TitleRole});
        });
        connect(view, &engine::EngineView::faviconUrlChanged, this, [this, view] {
            notifyViewChanged(view, {FaviconRole});
        });
        connect(view, &engine::EngineView::loadingChanged, this, [this, view] {
            notifyViewChanged(view, {LoadingRole});
        });
        connect(view, &engine::EngineView::loadProgressChanged, this, [this, view] {
            notifyViewChanged(view, {ProgressRole});
        });
