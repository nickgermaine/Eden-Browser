...
OmniboxController::OmniboxController(TabModel *tabs, HistoryStore *history, BookmarkStore *bookmarks, QObject *parent)
    ...
    connect(settings, &SettingsStore::searchEngineChanged, this, searchSettingsChanged);
    connect(settings, &SettingsStore::searchUrlChanged, this, searchSettingsChanged);
    connect(settings, &SettingsStore::searchSuggestionsChanged, this, searchSettingsChanged);
}
...
QUrl OmniboxController::destination(const QString &text, bool controlEnter) const {
    ...
    QString searchTemplate = SettingsStore::instance()->searchUrl();
...
void OmniboxController::sortAndLimit(QList<Suggestion> &suggestions) {
    std::stable_sort(suggestions.begin(), suggestions.end(),
                     [](const Suggestion &left, const Suggestion &right) { return left.score > right.score; });
...
void OmniboxController::rebuildLocalSuggestions() {
    ...
    if (!trimmed.isEmpty()) {
        ...
        suggestions.append(
            {"Search " + SettingsStore::instance()->searchEngine() + " for " + trimmed, destination(trimmed), "search", 900});
    }
...
bool OmniboxController::remoteSuggestionsAvailable() const {
    return SettingsStore::instance()->searchSuggestions() && remoteSuggestionProvider() != RemoteSuggestionProvider::None;
}
...
OmniboxController::RemoteSuggestionProvider OmniboxController::remoteSuggestionProvider() const {
    const QUrl searchUrl(SettingsStore::instance()->searchUrl());
...
void OmniboxController::updateCompletionSuffix() {
    ...
    if (!typed.isEmpty() && !typed.contains(' ') && m_suggestionsQuery == m_query && !m_suggestions.isEmpty()) {
        ...
        if (suggestion.kind != "search" && suggestion.kind != "remote") {
            ...
            const QString withoutWww = simplified.startsWith("www.", Qt::CaseInsensitive) ? simplified.sliced(4) : simplified;
