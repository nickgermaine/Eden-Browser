...
class ThemeManager final : public QAbstractListModel {
  ...
  private:
    ...
    std::optional<ThemeDefinition> parseTheme(const QByteArray &data, const QString &path, bool builtIn, QString &error) const;
    ...
    QJsonObject rootWithValue(const QJsonObject &root, const QStringList &parts, const QJsonValue &value, int index = 0) const;
