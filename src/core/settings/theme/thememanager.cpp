...
QHash<int, QByteArray> ThemeManager::roleNames() const {
    return {{IdRole, "themeId"}, {NameRole, "themeName"}, {AuthorRole, "themeAuthor"}, {BuiltInRole, "builtIn"}, {ActiveRole, "active"}};
}
...
QVariantMap ThemeManager::themePreview(const QString &id, bool dark) const {
    const auto theme =
        std::find_if(m_themes.cbegin(), m_themes.cend(), [&id](const ThemeDefinition &candidate) { return candidate.id == id; });
    ...
    preview.insert("toolbarGradientMiddlePosition", std::clamp(token(scheme, "chrome", "middlePosition").toDouble(0.5), 0.0, 1.0));
...
void ThemeManager::refresh() {
    ...
    for (const QFileInfo &fileInfo : files) {
        ...
        QString error;
        std::optional<ThemeDefinition> theme = parseTheme(file.readAll(), fileInfo.absoluteFilePath(), false, error);
        if (!theme) {
            if (firstError.isEmpty()) {
        ...
        const auto existing =
            std::find_if(themes.cbegin(), themes.cend(), [&theme](const ThemeDefinition &candidate) { return candidate.id == theme->id; });
...
bool ThemeManager::saveEditing() {
    ...
    if (m_editorBuiltIn) {
        ...
        const auto idExists = [this](const QString &candidate) {
            return std::any_of(m_themes.cbegin(), m_themes.cend(),
                               [&candidate](const ThemeDefinition &theme) { return theme.id == candidate; });
        };
        while (idExists(id)) {
...
bool ThemeManager::saveEditingAs() {
    ...
    const auto existing = std::find_if(m_themes.cbegin(), m_themes.cend(), [&id](const ThemeDefinition &theme) { return theme.id == id; });
...
QJsonObject ThemeManager::rootWithValue(const QJsonObject &root, const QStringList &parts, const QJsonValue &value, int index) const {
...
bool ThemeManager::writeEditorTheme(const QString &path) {
    ...
    QString error;
    const std::optional<ThemeDefinition> validated = parseTheme(QJsonDocument(root).toJson(QJsonDocument::Compact), path, false, error);
    if (!validated) {
        setLastError(error);
        return false;
    }
...
std::optional<ThemeManager::ThemeDefinition> ThemeManager::parseTheme(const QByteArray &data, const QString &path, bool builtIn,
                                                                      QString &error) const {
...
QJsonObject ThemeManager::migrateTheme(QJsonObject root) const {
    ...
    for (const QString &key : {QString("primary"), QString("onPrimary"), QString("primaryContainer"), QString("onPrimaryContainer"),
    ...
    for (const QString &key : {QString("lightColor"), QString("darkColor"), QString("ambientLightColor"), QString("ambientDarkColor")}) {
    ...
    for (const QString &scheme : {QString("light"), QString("dark")}) {
        ...
        for (const QString &key : {QString("primary"), QString("onPrimary"), QString("primaryContainer"), QString("onPrimaryContainer"),
                                   QString("privatePrimary"), QString("focusBorder"), QString("disabledIcon")}) {
            palette.remove(key);
        }
        QJsonObject chrome;
        ...
        const QJsonValue ambientColor = legacyShadow.value(scheme == "light" ? "ambientLightColor" : "ambientDarkColor");
...
QColor ThemeManager::paletteColor(const QString &key) const {
    static const QStringList baseColors = {"primary", "onPrimary", "primaryContainer", "onPrimaryContainer", "privatePrimary"};
    if (key == "focusBorder") {
        return paletteColor("primary");
    }
...
qreal ThemeManager::realToken(const QString &group, const QString &key, qreal minimum, qreal maximum) const {
    const qreal fallback = defaultTheme().root.value("base").toObject().value(group).toObject().value(key).toDouble();
...
QString ThemeManager::stringToken(const QString &group, const QString &key) const {
    const QString fallback = defaultTheme().root.value("base").toObject().value(group).toObject().value(key).toString();
