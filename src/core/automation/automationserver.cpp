...
std::unique_ptr<AutomationServer> AutomationServer::createIfEnabled(WindowController *controller, QQuickWindow *window, QObject *parent) {
    if (qEnvironmentVariable("EDEN_AUTOMATION") != "1") {
        return {};
    }
    auto server = std::make_unique<AutomationServer>(controller, window, qEnvironmentVariable("EDEN_AUTOMATION_SOCKET"), parent);
    if (!server->start()) {
        return {};
    }
    return server;
...
AutomationServer::AutomationServer(WindowController *controller, QQuickWindow *window, QString socketPath, QObject *parent,
    ...
    connect(m_window, &QQuickWindow::frameSwapped, this, [this] {
        ...
        if (PerformanceMetrics::frameCollectionEnabled()) {
            if (m_previousCollectedFrame.time_since_epoch().count() != 0) {
                PerformanceMetrics::record("frame.interval_ms",
...
AutomationServer::~AutomationServer() {
    if (qEnvironmentVariableIsSet("EDEN_IDLE_DIAGNOSTICS")) {
        qInfo("EDEN_PERF diagnostic.shell_frame_swaps=%llu", static_cast<unsigned long long>(m_idleDiagnosticFrameSwaps));
    }
    if (m_server->isListening()) {
        ...
        if (validateSocketParent()) {
            ...
            if (lstat(socketPath.constData(), &attributes) == 0 && S_ISSOCK(attributes.st_mode) && attributes.st_uid == geteuid()) {
...
bool AutomationServer::start() {
    if (!m_controller || !m_window || !validateSocketParent()) {
    ...
    if (lstat(socketPath.constData(), &attributes) == 0) {
        if (!S_ISSOCK(attributes.st_mode) || attributes.st_uid != geteuid() || !QLocalServer::removeServer(m_socketPath)) {
            return false;
        }
...
bool AutomationServer::validateSocketParent() const {
    ...
    struct stat attributes{};
    if (lstat(parentPath.constData(), &attributes) != 0 || !S_ISDIR(attributes.st_mode) || S_ISLNK(attributes.st_mode)) {
        return false;
    }
...
void AutomationServer::handleRequest(QLocalSocket *socket, const QByteArray &line) {
    ...
    if (method == "openTab") {
        ...
        if (from < 0 || to < 0 || from >= m_controller->tabs()->rowCount() || to >= m_controller->tabs()->rowCount() || from == to) {
            sendError(socket, id, -32602, "Invalid tab move");
            return;
        }
        ...
        result.insert("samples", PerformanceMetrics::samplesAfter(static_cast<quint64>(std::max<qint64>(0, after))));
        ...
        if (!parameters.value("path").isString() || !target.isAbsolute() || target.exists() ||
            target.suffix().compare("png", Qt::CaseInsensitive) != 0 || temporaryRoot.isEmpty() || targetParent.isEmpty() ||
        ...
        if (capture.isNull() || !output.open(QIODevice::WriteOnly) || !capture.save(&output, "PNG") || !output.commit()) {
        ...
        if (method == "pointerMove") {
            QMouseEvent event(QEvent::MouseMove, position, position, global, Qt::NoButton, Qt::NoButton, Qt::NoModifier);
            QCoreApplication::sendEvent(m_window, &event);
        } else {
            const Qt::MouseButton button = parameters.value("button").toString() == "right" ? Qt::RightButton : Qt::LeftButton;
            QMouseEvent press(QEvent::MouseButtonPress, position, position, global, button, button, Qt::NoModifier);
            QMouseEvent release(QEvent::MouseButtonRelease, position, position, global, button, Qt::NoButton, Qt::NoModifier);
        ...
        sendResult(socket, id, QJsonArray::fromVariantList(m_controller->navigationHistory(parameters.value("direction").toInt(-1))));
